...
export async function getGasReport(filters: {
    ...
    if (!session?.user?.role?.permissions.includes('view_solicitud_gas_report')) {
    ...
    try {
        ...
        const registrosRaw = await prisma.solicitudGas.findMany({
            where,
            orderBy: {
                fechaSolicitud: 'desc'
            }
        ...
        const enriched = await Promise.all(registrosRaw.map(async (reg: any) => {
        ...
        if (filters.sucursal) {
            finalResult = enriched.filter((r: any) => r.sucursal === filters.sucursal);
        }

        return { registros: finalResult, userSucursales: userSucursalNames }
    } catch (e) {
        console.error("Error fetching Gas Report:", e)
        return { error: 'Ocurrió un error al cargar el informe.' }
    }
}

export async function searchColegiosGasReport(query: string) {
    const session = await getSession()
    if (!session?.user?.role?.permissions.includes('view_solicitud_gas_report')) {
    ...
    try {
        ...
        console.error("Error searching colegios for gas report:", e)
