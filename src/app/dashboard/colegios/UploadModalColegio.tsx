...
import { uploadColegiosData, checkColegiosExists, ColegioData } from './actions'
import { useRouter } from 'next/navigation'

export default function UploadModalColegio() {
    ...
    const [parsedData, setParsedData] = useState<ColegioData[]>([])
    const router = useRouter()

    const fileInputRef = useRef<HTMLInputElement>(null)

    const expectedColumns = ['colut', 'colrbd', 'colrbddv', 'insid', 'institución', 'sucursal', 'nombreestablecimiento', 'direccionestablecimiento', 'comuna']
    ...
    const validateAndParseExcel = async () => {
        ...
        try {
            ...
            if (missingCols.length > 0) {
                // Alternativa a "institución" tildada, chequear sin tilde si no existe
                if (missingCols.includes('institución') && headers.includes('institucion')) {
                    const idx = missingCols.indexOf('institución')
                    missingCols.splice(idx, 1)
                }

                if (missingCols.length > 0) {
                    setError(`Faltan columnas obligatorias: ${missingCols.join(', ')}. Estructura esperada original: ${expectedColumns.join(', ')}`)
                    setLoading(false)
                    return
                }
            }
            ...
            // Mapeo
            const formattedData: ColegioData[] = normalizedData.map(row => {
                return {
                    colut: Number(row['colut']) || 0,
                    colRBD: Number(row['colrbd']) || 0,
                    colRBDDV: String(row['colrbddv'] || '').substring(0, 20),
                    insid: String(row['insid'] || '').substring(0, 20),
                    institucion: String(row['institución'] || row['institucion'] || '').substring(0, 20),
                    sucursal: String(row['sucursal'] || '').substring(0, 50),
                    nombreEstablecimiento: String(row['nombreestablecimiento'] || '').substring(0, 200),
                    direccionEstablecimiento: String(row['direccionestablecimiento'] || '').substring(0, 200),
                    comuna: String(row['comuna'] || '').substring(0, 100),
                }
            })

            // Filtrar filas inválidas (ej. sin RBD)
            const cleanData = formattedData.filter(d => d.colRBD > 0)

            if (cleanData.length === 0) {
                setError('El archivo no contiene colegios o los datos de ColRBD no tienen formato Int numérico válido.')
                setLoading(false)
                return
            }
            ...
            const validation = await checkColegiosExists(cleanData)
    ...
    const executeUpload = async (data: ColegioData[], overwrite: boolean) => {
        setLoading(true)
        setError('')
        const result = await uploadColegiosData(data, overwrite)
        if (result.error) {
            setError(result.error)
        } else {
            setSuccess(`Se cargaron ${result.count} colegios exitosamente.`)
            setFile(null)
                    ...
                    {!confirmOverwrite ? (
                        ...
                        <div className="bg-yellow-50 border border-yellow-200 p-6 rounded-2xl animate-in slide-in-from-bottom-4">
                            <h4 className="text-lg font-bold text-yellow-800 flex items-center gap-2 mb-2">
                                ⚠️ Colegios Existentes
                            </h4>
                            <p className="text-sm text-yellow-700 mb-6">
                                Hemos detectado que ya existen colegios registrados con los mismos IDs identificados (RBD).
                                <br /><br />
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {loading ? 'Sobrescribiendo...' : 'Sí, actualizar colegios'}
                                </button>
                            </div>
                        </div>
                    )}
