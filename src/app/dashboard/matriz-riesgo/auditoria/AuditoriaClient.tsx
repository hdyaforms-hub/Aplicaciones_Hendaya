...
const SECCION_COLORS: Record<string, { bg: string, text: string }> = {
    'PATIO_SERVICIO': { bg: 'bg-amber-100', text: 'text-amber-800' },
    'BODEGA': { bg: 'bg-blue-100', text: 'text-blue-800' },
    'COCINA': { bg: 'bg-orange-100', text: 'text-orange-800' },
    'BANO': { bg: 'bg-teal-100', text: 'text-teal-800' },
    'LEVANTAMIENTO_GENERAL': { bg: 'bg-purple-100', text: 'text-purple-800' }
}
...
export default function AuditoriaClient({
    ...
    const [selectedRespuesta, setSelectedRespuesta] = useState<any | null>(null)
    ...
    // Enriquecer respuestas con nombre de colegio y conteo de hallazgos
    const enrichedRespuestas = useMemo(() => {
        return respuestas.map(r => {
            const colegio = colegios.find(c => c.colRBD === r.rbd)
            ...
            r.detalles.forEach((d: any) => {
                if (d.valor && d.valor !== '') {
                    totalPreguntas++
                    if (PROBLEM_VALUES.includes(d.valor)) {
                        hallazgos++
                    }
                }
            })
    ...
    useEffect(() => {
        if (selectedRespuesta && !enrichedRespuestas.some(r => r.id === selectedRespuesta.id)) {
            setSelectedRespuesta(null)
        }
    }, [enrichedRespuestas, selectedRespuesta])

    // Extraer fotos de la respuesta para una sección
    const getSectionPhotos = (respuesta: any, seccion: string, configDetalles: any[]) => {
        const fotos: string[] = []
        respuesta.detalles.forEach((d: any) => {
            const configPregunta = configDetalles.find(p => p.id === d.preguntaId)
            if (configPregunta && configPregunta.seccion === seccion && d.adjuntoUrl) {
                // If it's stored as JSON string or single URL
                try {
                    const parsed = JSON.parse(d.adjuntoUrl)
                    if (Array.isArray(parsed)) {
                        fotos.push(...parsed)
                    } else {
                        fotos.push(d.adjuntoUrl)
                    }
                } catch {
                    fotos.push(d.adjuntoUrl)
                }
    ...
    const handleExportExcel = async () => {
        ...
        const config = cabecerasConfig.find(c => c.id === selectedRespuesta.cabeceraId)
        if (!config) return

        const row: any = {
            'UT': selectedRespuesta.ut,
            'RBD': selectedRespuesta.rbd,
            'Nombre_establecimiento': selectedRespuesta.colegioNombre,
            'Fecha': format(new Date(selectedRespuesta.createdAt), 'dd/MM/yyyy')
        }

        config.detalles.filter((p: any) => p.tipoRespuesta !== 'ADJUNTAR').forEach((p: any) => {
            const respuestaObj = selectedRespuesta.detalles.find((d: any) => d.preguntaId === p.id)
            row[p.preguntaNombre] = respuestaObj?.valor || 'Sin Respuesta'
        })

        const secciones = Array.from(new Set(config.detalles.map((d: any) => d.seccion)))
        secciones.forEach((s: any) => {
            row[`Foto ${s}`] = getSectionPhotos(selectedRespuesta, s, config.detalles).length > 0 ? 'si' : 'no'
        })

        const wb = XLSX.utils.book_new()
        const ws = XLSX.utils.json_to_sheet([row])
        XLSX.utils.book_append_sheet(wb, ws, "Auditoria")
        XLSX.writeFile(wb, `Auditoria_RBD_${selectedRespuesta.rbd}.xlsx`)
    }
    ...
    const handleExportPDF = async () => {
        ...
        doc.setFontSize(16)
        doc.text(`Auditoría - ${selectedRespuesta.colegioNombre} (RBD: ${selectedRespuesta.rbd})`, 14, 20)
        doc.setFontSize(10)
        doc.text(`Fecha Evaluación: ${format(new Date(selectedRespuesta.createdAt), 'dd/MM/yyyy')}`, 14, 28)
        ...
        secciones.forEach((sec: any) => {
            const preguntasSeccion = config.detalles.filter((p: any) => p.seccion === sec && p.tipoRespuesta !== 'ADJUNTAR')
            const fotos = getSectionPhotos(selectedRespuesta, sec, config.detalles)
            
            const tableData = preguntasSeccion.map((p: any) => {
                const resObj = selectedRespuesta.detalles.find((d: any) => d.preguntaId === p.id)
                return [p.preguntaNombre, resObj?.valor || 'Sin Respuesta']
            })
            
            tableData.push([`¿Existen fotos de evidencia para ${sec}?`, fotos.length > 0 ? 'Sí' : 'No'])

            autoTable(doc, {
                startY: currentY,
                head: [[sec, 'Respuesta / Estado']],
                body: tableData,
                theme: 'grid',
        ...
        doc.save(`Auditoria_RBD_${selectedRespuesta.rbd}.pdf`)
    }

    const handleExportAllExcel = async () => {
        if (enrichedRespuestas.length === 0) return

        const XLSX = await import('xlsx')

        const rows = enrichedRespuestas.map(r => {
            const config = cabecerasConfig.find(c => c.id === r.cabeceraId)
            const row: any = {
                'UT': r.ut,
                'RBD': r.rbd,
                'Nombre_establecimiento': r.colegioNombre,
                'Fecha': format(new Date(r.createdAt), 'dd/MM/yyyy')
            }

            if (config) {
                config.detalles.filter((p: any) => p.tipoRespuesta !== 'ADJUNTAR').forEach((p: any) => {
                    const resObj = r.detalles.find((d: any) => d.preguntaId === p.id)
                    row[p.preguntaNombre] = resObj?.valor || 'Sin Respuesta'
                })

                const secciones = Array.from(new Set(config.detalles.map((d: any) => d.seccion)))
                secciones.forEach((s: any) => {
                    row[`Foto ${s}`] = getSectionPhotos(r, s, config.detalles).length > 0 ? 'si' : 'no'
                })
            }
    ...
    if (selectedRespuesta) {
        const config = cabecerasConfig.find(c => c.id === selectedRespuesta.cabeceraId)
        const secciones = config ? Array.from(new Set(config.detalles.map((d: any) => d.seccion))) : []
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-md border border-slate-200 mb-8 mt-2 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <h2 className="text-3xl font-black text-slate-800 tracking-tight leading-tight">
                            {selectedRespuesta.colegioNombre} <span className="text-slate-400 font-medium text-xl block sm:inline">| RBD: {selectedRespuesta.rbd}</span>
                        </h2>
                        <p className="text-xs font-black text-slate-500 mt-2 uppercase tracking-widest bg-slate-100 inline-block px-3 py-1 rounded-full">
                            Evaluado el {format(new Date(selectedRespuesta.createdAt), 'dd/MM/yyyy')}
                        </p>
                    </div>
                    <div className="flex flex-col items-end gap-3 w-full md:w-auto">
                        <div className={`text-4xl font-black bg-slate-50 px-6 py-2 rounded-2xl border ${selectedRespuesta.hallazgos > 0 ? 'text-red-500 border-red-100' : 'text-emerald-500 border-emerald-100'}`}>
                            {selectedRespuesta.hallazgos} <span className="text-lg text-slate-400 font-medium tracking-normal">Hallazgos</span>
                        </div>
                ...
                <div className="space-y-12">
                    {secciones.map((secName: any) => {
                        const preguntasSeccion = config.detalles.filter((p: any) => p.seccion === secName && p.tipoRespuesta !== 'ADJUNTAR')
                        const fotosOriginales = getSectionPhotos(selectedRespuesta, secName, config.detalles)
                        const colors = SECCION_COLORS[secName] || { bg: 'bg-slate-100', text: 'text-slate-800' }

                        return (
                            <div key={secName} className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                                <div className={`px-8 py-5 flex justify-between items-center ${colors.bg}`}>
                                    <h3 className={`text-lg font-black uppercase tracking-widest ${colors.text}`}>
                                        {secName.replace(/_/g, ' ')}
                                    </h3>
                                ...
                                <div className="divide-y divide-slate-50">
                                    {preguntasSeccion.map((p: any) => {
                                        const resObj = selectedRespuesta.detalles.find((d: any) => d.preguntaId === p.id)
                                        const responseValue = resObj?.valor || 'Sin Respuesta'
                                        const isProblem = PROBLEM_VALUES.includes(responseValue)
                                        const mitigacion = mitigaciones.find(mit => mit.matrizId === selectedRespuesta.id && mit.preguntaId === p.id)
                                        ...
                                        return (
                                            <div key={p.id} className="p-8 hover:bg-slate-50/50 transition-colors flex flex-col lg:flex-row gap-6">
                                                <div className="flex-1">
                                                    <p className="text-slate-800 font-bold text-base leading-relaxed">{p.preguntaNombre}</p>
    ...
    return (
        <div className="animate-in fade-in duration-500 space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex-col md:flex-row gap-4">
                <div className="flex items-center gap-4 w-full md:w-auto px-2">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Año de Auditoría:</p>
                    <select 
                        value={selectedYear}
                ...
                {!error && (
                    <div className="flex justify-between items-center w-full md:w-auto">
                        <p className="text-slate-500 font-bold px-4">Mostrando <span className="text-slate-800">{enrichedRespuestas.length}</span> evaluaciones</p>
                        {enrichedRespuestas.length > 0 && (
                            <button 
            ...
            {error ? (
                ...
                <div className="bg-white p-20 rounded-[3rem] shadow-sm border border-slate-100 text-center">
                    ...
                    <p className="text-slate-500 font-medium">No se han ingresado evaluaciones aún.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {enrichedRespuestas.map(r => (
                        <div 
                            key={r.id}
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm font-bold mt-1 tracking-widest">RBD {r.rbd}</p>
                                </div>
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 border-2 ${r.hallazgos > 0 ? 'bg-red-50 text-red-500 border-red-100 group-hover:bg-red-500 group-hover:text-white group-hover:border-red-500' : 'bg-emerald-50 text-emerald-500 border-emerald-100 group-hover:bg-emerald-500 group-hover:text-white group-hover:border-emerald-500'} transition-colors`}>
                                    <span className="font-black text-lg">{r.hallazgos}</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 pt-6 border-t border-slate-100">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className="text-xs font-black text-slate-800">{r.cumplimiento.toFixed(1)}%</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center mt-4 text-[10px] font-black uppercase text-slate-400 tracking-wider">
                                    <span>{format(new Date(r.createdAt), 'dd MMMM yyyy')}</span>
