        ...
        <div className="max-w-6xl mx-auto pb-12">
            ...
            {filteredForms.length === 0 ? (
                ...
                <div className="space-y-12">
                    {/* Grouping Logic */}
                    {Object.entries(
                        filteredForms.reduce((acc: Record<string, any[]>, form) => {
                            const areaName = form.area?.nombre || 'General / Sin Área'
                            if (!acc[areaName]) acc[areaName] = []
                            acc[areaName].push(form)
                            return acc
                        }, {})
                    ).map(([areaName, areaForms]) => (
                        <div key={areaName} className="space-y-6">
                            <div className="flex items-center gap-4">
                                <h3 className="text-lg font-black text-slate-800 uppercase tracking-widest px-4 py-1.5 bg-slate-100 rounded-xl border border-slate-200 shadow-sm flex items-center gap-2">
                                    <span className="text-xl">🏢</span> {areaName}
                                    <span className="ml-2 bg-slate-900 text-white text-[10px] px-2 py-0.5 rounded-full">{areaForms.length}</span>
                                </h3>
